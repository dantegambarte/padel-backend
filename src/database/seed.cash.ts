...
const dataSource = new DataSource({
  ...
  logging:     false,
  entities: [
    User, Court, ProductCategory, Product,
...
interface ShiftSpec {
  ...
  openHh:       number;
  openMm:       number;
  /** null = sesión OPEN (turno actual sin cerrar). */
...
interface DaySpec {
  date:    string;   // fecha comercial YYYY-MM-DD
  morning: ShiftSpec;
  night:   ShiftSpec;
}
...
async function seedCash(): Promise<void> {
  ...
  // ══════════════════════════════════════════════════════════════════════════
  // PASO A — Resolver dependencias (fuera de transacción)
  // ══════════════════════════════════════════════════════════════════════════
  ...
  if (allUsers.length === 0) {
    ...
    allUsers = await userRepo.save([
      { username: 'gustavo', fullName: 'Gustavo Sosa',     passwordHash: h1, role: UserRole.EMPLOYEE, isActive: true },
      { username: 'guada',   fullName: 'Guadalupe García', passwordHash: h2, role: UserRole.EMPLOYEE, isActive: true },
    ]);
  ...
  if (products.length === 0) {
    ...
    products = await productRepo.save([
      { name: 'Agua Mineral 500ml', category: cat, categoryId: cat.id, costPrice: 150, salePrice: 300, stock: 48, minStock: 10, isFeatured: true, isActive: true },
      { name: 'Gatorade',           category: cat, categoryId: cat.id, costPrice: 250, salePrice: 450, stock: 30, minStock: 10, isFeatured: true, isActive: true },
    ]);
  ...
  // ══════════════════════════════════════════════════════════════════════════
  // PASO B — Definición del escenario de 3 días
  // ══════════════════════════════════════════════════════════════════════════

  const D3 = dateOffset(-3);   // hace 3 días
  const D2 = dateOffset(-2);   // hace 2 días
  const D1 = dateOffset(-1);   // ayer (último día histórico)
  ...
  const days: DaySpec[] = [
    // ── Día -3 ────────────────────────────────────────────────────────────
    {
      date: D3,
      morning: {
        user: 'user1', openHh: 9, openMm: 0,
        closeDate: D3, closeHh: 18, closeMm: 0,
        sales: [
        ...
        // cashExpected = 2000+1500+7500 = 11000 → counted 11200 → Δ +200
        cashCounted: 11200, difference: 200,
        notes: 'Turno mañana — seed:cash',
      },
      night: {
        user: 'user2', openHh: 18, openMm: 0,
        closeDate: D2, closeHh: 1, closeMm: 0,
        sales: [
          { cash: 1500, tr:    0, customer: 'Molina' },
          { cash:    0, tr: 2500, customer: 'Ponce'  },
        ],
        bookings: [
          { hour: '19:00', client: 'Ibáñez', priceAmount: 7500, cash: 7500, tr: 0 },
        ],
        // cashExpected = 1500+7500 = 9000 → counted 8700 → Δ -300
        cashCounted: 8700, difference: -300,
    ...
    // ── Día -2 ────────────────────────────────────────────────────────────
    {
      ...
      morning: {
        user: 'user1', openHh: 9, openMm: 0,
        closeDate: D2, closeHh: 18, closeMm: 0,
        sales: [
        ...
        // cashExpected = 2500+2000+8000 = 12500 → counted 12500 → Δ 0
        cashCounted: 12500, difference: 0,
        notes: 'Turno mañana — seed:cash',
      },
      night: {
        user: 'user2', openHh: 18, openMm: 0,
        closeDate: D1, closeHh: 1, closeMm: 0,
        sales: [
          { cash: 1800, tr:    0, customer: 'Fuentes' },
          { cash:    0, tr: 2000, customer: 'Paredes'  },
        ],
        bookings: [
          { hour: '19:00', client: 'Romero', priceAmount: 8500, cash: 8500, tr: 0 },
        ],
        // cashExpected = 1800+8500 = 10300 → counted 10000 → Δ -300
        cashCounted: 10000, difference: -300,
        notes: 'Turno noche — seed:cash',
      },
    },
    ...
    // ── Día -1 (ayer) ─────────────────────────────────────────────────────
    {
      ...
      morning: {
        user: 'user1', openHh: 9, openMm: 0,
        closeDate: D1, closeHh: 18, closeMm: 0,
        sales: [
        ...
        // cashExpected = 3000+1500+8000 = 12500 → counted 12700 → Δ +200
        cashCounted: 12700, difference: 200,
        notes: 'Turno mañana — seed:cash',
      },
      night: {
        user: 'user2', openHh: 18, openMm: 0,
        closeDate: null, closeHh: null, closeMm: null,
        sales: [
          { cash: 1800, tr:    0, customer: 'Torres' },
          { cash:    0, tr: 2200, customer: 'Ruiz'   },
        ],
        bookings: [
          { hour: '19:00', client: 'Fernández', priceAmount: 8000, cash: 8000, tr: 0 },
        ],
        // OPEN — cashCounted = null, difference = null
        cashCounted: null, difference: null,
        notes: 'Turno noche — seed:cash',
      },
    },
  ];
  ...
  // ══════════════════════════════════════════════════════════════════════════
  // PASO C — Limpieza idempotente en orden FK-safe
  // ══════════════════════════════════════════════════════════════════════════
  ...
  // Sesiones OPEN de cualquier fecha
  ...
  // Sesiones de los 3 días objetivo
  ...
  // ══════════════════════════════════════════════════════════════════════════
  // PASO D — Insertar los 3 días en una sola transacción
  // ══════════════════════════════════════════════════════════════════════════
  ...
  try {
    ...
    const insertSession = async (
      ...
      const [row] = await qr.query(
        ...
        [
          date, user.id,
          spec.cashCounted, spec.difference, spec.notes,
    ...
    const insertSale = async (
      sessionId: string, user: User, s: SaleSpec,
    ): Promise<Sale> => {
      ...
      // 1 item de cada producto por venta (datos de demostración)
    ...
    const insertTx = (
      ...
      type: TransactionType, refId: string,
      concept: string, cash: number, tr: number,
    ...
    const upsertBooking = async (
      b: BookingSpec, date: string, userId: string,
    ): Promise<string> => {
    ...
    for (const day of days) {
      ...
      for (const [label, spec] of [['Mañana', day.morning], ['Noche', day.night]] as [string, ShiftSpec][]) {
        ...
        for (const bSpec of spec.bookings) {
          ...
          await insertTx(sId, user, TransactionType.BOOKING, bId,
            `${court.name} · ${bSpec.hour}hs — ${bSpec.client}`, bSpec.cash, bSpec.tr);
        }

        const cashExp = spec.sales.reduce((s, x) => s + x.cash, 0)
                      + spec.bookings.reduce((s, x) => s + x.cash, 0);
        const trExp   = spec.sales.reduce((s, x) => s + x.tr, 0)
                      + spec.bookings.reduce((s, x) => s + x.tr, 0);

        if (spec.cashCounted !== null) {
          const sign  = spec.difference === 0 ? 'Δ $0 (cuadra)' : `Δ ${spec.difference! > 0 ? '+' : ''}$${spec.difference}`;
          console.log(`         ef.$${cashExp} | tr.$${trExp} | contado $${spec.cashCounted} | ${sign}`);
        } else {
    ...
    for (const day of days) {
      for (const [label, spec] of [['Mañana', day.morning], ['Noche', day.night]] as [string, ShiftSpec][]) {
        ...
        const diff     = spec.difference  === null ? '    —     '
                       : spec.difference  === 0    ? '   $0     '
                       : spec.difference! > 0      ? `  +$${spec.difference}  `
                       :                             `  -$${Math.abs(spec.difference!)}  `;
        console.log(`║ ${day.date.slice(5)} ║ ${label.padEnd(9)} ║ ${estado} ║ $${String(cashExp).padEnd(7)} ║ ${contado.padEnd(7)} ║ ${diff.padEnd(10)} ║`);
      }
    }
